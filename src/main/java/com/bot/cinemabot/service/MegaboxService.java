	...
	public void aJob() throws IOException {
		...
		if (!onePlusOneTickets.isEmpty() && isChangedTicket) {
			...
			if (!StringUtils.isEmpty(new1p1Tickets.getName())) {
				...
				MessageFormat format = new MessageFormat("메가박스", newTickets.getName(), newTickets.getDate(), newTickets.getPrice(), String.valueOf(onePlusOneTickets.size()), String.valueOf(allTickets.size()), newTickets.getLink(), true);
