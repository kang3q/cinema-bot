...
public class CinemaScheduler {
	...
	@Scheduled(cron = "0 0/2 08-20 ? * MON-FRI") // 월~금, 매월, 아무 날이나, 08:00 ~ 20:59, 2분마다, 0초에
	public void lotteCinema() {
		lotteCinemaService.aJob();
	}

	@Scheduled(cron = "20 0/2 08-20 ? * MON-FRI") // 월~금, 매월, 아무 날이나, 08:00 ~ 20:59, 2분마다, 20초에
	public void cgv() throws IOException {
		cgvService.aJob();
	}

	@Scheduled(cron = "40 0/2 08-20 ? * MON-FRI") // 월~금, 매월, 아무 날이나, 08:00 ~ 20:59, 2분마다, 40초에
